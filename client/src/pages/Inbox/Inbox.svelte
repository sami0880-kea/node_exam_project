          ...
          <li class="py-3 sm:py-4">
            <button class="flex items-start space-x-3 cursor-pointer"  on:click={() => goToConversation(room._id)}>
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-left text-sm text-gray-500 truncate dark:text-gray-400">{room.latestMessage.text.length > 24 ? room.latestMessage.text.substring(0, 24) + '...' : room.latestMessage.text}</p>
              </div>
            </button>
          </li>
        {/each}
